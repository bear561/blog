      ...
      >
        ...
        <div class="comment-body">
          ...
          <div class="comment-actions">
            <el-button text size="small" type="primary" @click="$emit('reply', comment)">
              回复
            </el-button>
          </div>
          <div v-if="comment.children && comment.children.length" class="nested-comments">
            <CommentList :comments="comment.children" @reply="$emit('reply', $event)" />
          </div>
        </div>
      </div>
    </div>
...
.comment-item.is-reply {
  margin-left: 48px; padding: 14px 16px;
...
.nested-comments { margin-top: 8px; }
</style>
